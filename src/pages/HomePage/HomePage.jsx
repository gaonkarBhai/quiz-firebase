        ...
        <div className="hp-container">
          ...
          <div className="homepage-body">
            <section className='dashboard-hero'>
              <h2>Ongoing <span className="event_year">Events</span></h2>
              {user ? (
                <></>
              ) : (
          ...
          <div className="quizes_container">
            <div className="quiz_cards_stack">
              {user ? (
                quizzes.length > 0 ? (
                  quizzes.map((ele, i) => (
                      <div className="quiz-box" key={ele.id}>
                        <img src={ele.imgUrl} alt={ele.title} className="quiz-img" />
                        ...
                        <div className="quiz_footer">
                          <p className="quiz_TimeLimit">{ele.timeLimit} <span style={{ color: 'black' }}>min</span></p>
                          <a className="hp-quiz-btn" href={`/user-rules/${ele.ID}`}>
                            Start
                          </a>
                        </div>
                      </div>
